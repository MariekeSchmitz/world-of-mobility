...
public class NpcPlacementValidator implements Validator {
    ...
    public boolean validate() {
        ...
        if (npcToValidate instanceof Passenger && !(tileToPlaceOn instanceof Walkable)) {

            logger.error("npc cannot be placed. Passenger npc cant be put on non walkable tile.");
            return false;

        } else if (npcToValidate instanceof MotorizedObject && !(tileToPlaceOn instanceof DriveableByCar)) {
        ...
        for (MoveableObject npc : npcs) {
            ...
            if (xPos == npc.getXPos() && yPos == npc.getYPos()) {
                logger.error("npc cannot be placed. Already another npc on position {} {}.", npcToValidate.getXPos(),
                        npcToValidate.getYPos());
                return false;
            }
        }
